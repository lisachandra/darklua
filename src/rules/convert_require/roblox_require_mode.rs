...
fn parse_roblox_expression(
    ...
    match expression {
        ...
        Expression::String(x) => {
            handle_roblox_script_parent(x.get_string_value().unwrap_or_default(), path_builder, current_path)?
        }
        _ => Err(
...
fn handle_roblox_script_parent(
    ...
    match str {
        ...
        x @ "Parent" => {
            current_path.pop();
            path_builder.push_back(x.to_string());
        }
        x => path_builder.push_back(x.to_string()),
    };
    Ok(())
}
